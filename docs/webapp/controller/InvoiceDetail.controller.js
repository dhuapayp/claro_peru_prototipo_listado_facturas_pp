sap.ui.define([
	...
	return Controller.extend("claro.com.listadofacturas.controller.InvoiceDetail", {
		...
		_onRouteMatched: function (oEvent) {
			...
			// Determine mode: respect explicit query param, fall back to status
			var oQuery = oEvent.getParameter("arguments")["?query"] || {};
			var sRequestedMode = oQuery.mode; // "view" | "edit" | undefined
			var bEditable = (oInvoice.Status === "Registrado" || oInvoice.Status === "Rechazado");
			// If user clicked "view", force read-only even if status allows editing
			var bReadOnly = sRequestedMode === "view" ? true : !bEditable;
			var sMode = bReadOnly ? "view" : "edit";
			...
			setTimeout(function () {
				...
				if (oWizard && oStep1 && oLastStep) {
					try {
						...
						if (bReadOnly) {
							// Silently unlock all steps without scrolling
							oWizard.discardProgress(oStep1);
							[oStep2, oStep3, oLastStep].forEach(function (s) {
								oWizard.nextStep();
							});
							oWizard.goToStep(oStep1, true);
						} else {
							// Edit mode: unlock all steps, start at step 1
							oWizard.discardProgress(oStep1);
							[oStep2, oStep3, oLastStep].forEach(function (s) {
								oWizard.nextStep();
							});
		...
		_loadContratoStep3: function (oDetail) {
			var bReadOnly = this.getView().getModel("viewModel").getProperty("/readOnly");
			...
			// Pendientes por facturar — in read-only show only assigned
			var aPend = JSON.parse(JSON.stringify(oDetail.pendientesFact || []));
			if (bReadOnly) {
				aPend = aPend.filter(function (o) { return o.asignado; });
			}
			this._allPendientesData = aPend;
			...
			var that = this;
			setTimeout(function () { that._refreshPendientesHighlight(); }, 200);
		},
		...
		_loadOCStep3: function (oDetail) {
			...
			if (bReadOnly) {
				aAll = aAll.filter(function (o) { return o.asignado; });
			}
			this._allPosicionesData = aAll;
			this._applyPosicionesPagination(1);
			var that = this;
			setTimeout(function () { that._refreshPosicionesHighlight(); }, 200);
		},
		...
		onPendientesRowsUpdated: function () {
			this._refreshPendientesHighlight();
		},
				...
				" con estado \"REGISTRADO\".\n\nAsignaciones: " + aAsig.length + "\n¿Desea continuar?",
				{
					...
					onClose: function (oAction) {
						if (oAction === MessageBox.Action.OK) {
							...
							that.getOwnerComponent().getRouter().navTo("InvoiceList");
						}
					}
				}
			);
		...
		onWizardStepActivate: function (oEvent) {
			...
			if (sStepId && sStepId.indexOf("wizardStep3") >= 0) {
				...
				// Extra pass for t:Table rows that render late
				setTimeout(function () {
					that._refreshPendientesHighlight();
				}, 500);
			}
